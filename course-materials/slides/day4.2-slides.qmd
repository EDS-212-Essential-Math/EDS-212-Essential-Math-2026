...
## {#probability-title data-menu-title="Summation Notation" background-color="#003660"}
...
- Experiment: Checking whether it rained on a given day
- Event space: {rain, no rain}
- Event: $A =$ it rains today
- $P(A)=$ What is the probability that it rains today?
...
**Independent events:** two consecutive coin flips: one has no bearing on the other.

**Non-independent events:** in our water sample, exceeding the nitrate limit and exceeding the E. coli limit could be non-independent. Perhaps both are often driven by the same upstream agricultural runoff, so if $A$ occurs, $B$ becomes more likely too.
...
[Basic probability theory]{.slide-title2}
...
::: {.body-text-m}
- **Intersection:** Probability that events co-occur. In other words, these are **AND** probability statements.

<br>

- **Union:** Probability that *at least one* of the events happens (could be just one, or all). In other words, these are **OR** probability statements. 
